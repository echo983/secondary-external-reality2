...
Real calls go to Cloudflare Workers AI. **Only Cloudflare-self-hosted models are approved (anything under the `@cf/...` namespace on Workers AI) — no third-party API (OpenAI, Anthropic, etc.), no model hosted anywhere else.** (Loosened 2026-08-29 from "only `@cf/qwen/qwen3.8-27b`" — see `docs/architecture-direction-consensus-2026-08-28.md` §13. The point was never "exactly one model," it was "stay on Cloudflare's own infrastructure, don't silently fall back.") Which specific model backs which role is a deliberate choice made via real A/B testing against that role's validated cases (`docs/*-findings-*.md`), never a runtime fallback — a role must not silently retry on a different model when its assigned model errors or times out; the failure surfaces as-is. Currently every role still runs on `@cf/qwen/qwen3.8-27b`; nothing has actually been switched yet. Put an API token in `secret/cftoken.txt` (gitignored). AI Search (RAG) calls use the same token against instance `sr2-truth-store`.
...
- **Cloudflare-self-hosted models only, chosen deliberately per role, never a runtime fallback**: any `@cf/...` Workers AI model is allowed; switching a role to a different model requires real A/B testing against that role's validated cases first, not a guess. Currently every role runs `@cf/qwen/qwen3.8-27b`.
