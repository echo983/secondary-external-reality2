import type {CanonicalFact, ProcessState, TruthCell, WorldBasis, WorldSnapshot} from "../domain/types.js";
import type {CollapseRegistration} from "../protocol/collapse-policy.js";
...
export interface DemoFixture {
  ...
  collapseRegistrations: readonly CollapseRegistration[];
}
...
export function createDemoFixture(): DemoFixture {
  const worldBasis = {worldId: "text-vr-demo", schemaVersion: "1", fixtureVersion: "demo-v4", genesisHash: "demo-genesis-v4"};
  const entities: FixtureEntity[] = [
    ...
    {entityId: "floor-1", kind: "object", aliases: ["地面", "地板"], perceivableBy: ["self"]},
  ...
  const facts = [
    ...
    fact("g-floor-place", "placement:floor-1", "bedroom"),
  ...
  const truthCells: TruthCell[] = [{address: "fit:blanket-1:under_gap:door-1", domain: [true, false],
    constraints: [], revision: 1}];
  const collapseRegistrations: CollapseRegistration[] = [{address: truthCells[0]!.address,
    allowedDomain: [true, false], radius: "local"}];
  const affordances = {
    ...
    "bed-1": ["contact", "support", "contains"], "floor-1": ["contact", "support", "contains"],
    "blanket-1": ["contact", "hold", "release", "place", "move", "relation:held_by", "relation:placed_at", "relation:occludes", "deformable"]
  } as const;
  return {worldBasis, entities, genesis: createGenesis(worldBasis, "2026-08-27T18:24:00.000Z", {facts, processes, truthCells}),
    affordances, collapseRegistrations, allowedRelations: ["open", "aperture_cm", "held_by", "placed_at", "occludes"]};
}
