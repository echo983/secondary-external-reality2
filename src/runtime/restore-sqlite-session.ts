...
export async function restoreSqliteSession(options: {
  ...
  try {
    ...
    for (const commit of store.pending(options.actorId)) {
      ...
      } else if (commit.delta.events.some(event => ["object_held", "object_released", "object_placed", "object_dragged", "placement_blocked", "actor_moved", "actor_approached", "actor_oriented", "speech"].includes(event.kind))) {
