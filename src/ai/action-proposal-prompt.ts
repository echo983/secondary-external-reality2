...
export const ACTION_PROPOSAL_JSON_SCHEMA = {
  ...
  required: ["clauseIndex", "primitives", "targetSlots", "conditions", "effects", "perceptionScopes", "unresolvedDependencies"],
  properties: {
...
只输出一个 JSON 对象，严格字段为 clauseIndex、primitives、targetSlots、conditions、effects、perceptionScopes、durationSeconds（可省略）、unresolvedDependencies。
...
例1 输入“看看四周”：primitives=["perceive"], targetSlots=[], effects=[{"kind":"observation_scope","subjectSlot":"actor","field":"vision","certainty":"required"}], perceptionScopes=[{"modality":"vision","originSlot":"actor","horizon":"ambient","targetSlots":[]}], 其余数组为空。
