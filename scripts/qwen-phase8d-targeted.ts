...
const allInputs = ["松开毛毯", "大声喊“有人吗”", "再次拿起毛毯然后放到床上"] as const;
const inputs: readonly string[] = process.argv.includes("--release-only") ? allInputs.slice(0, 1) : allInputs;
...
for (const input of inputs) {
  ...
  try {
    ...
    results.push({input, status: "constituted", kind: proposal.kind, primitives: proposal.primitives,
      operations: constituted.clauses.map(clause => clause.operation), telemetry: model.telemetry()});
  } catch (cause) {
