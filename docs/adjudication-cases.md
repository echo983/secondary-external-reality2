...
状态：revised after direction review
日期：2026-08-27
...
E01–E07 定义为扩展池，不计入正式基线。原可信内核集合为 I01–I10、C01–C08、P01–P10、T01–T07、L01–L07、F01–F08，共 50；扩展池在对应能力进入实现时启用。

## 8A. 开放交互纠偏案例（O01–O10）

以下十项加入正式基线，专门防止可信内核退化为固定命令集合：

| ID | 初始/触发 | 预期 | 禁止 | 测试层 |
|---|---|---|---|---|
| O01 | self 初次进入卧室 | 自动投递当前位置第一屏；来源闭合 | 空白开局；要求玩家先猜命令 | perception integration |
| O02 | 输入“看看四周”且无 target mention | scope 解析为 self 的视觉 horizon | TARGET_UNGROUNDED；虚构“四周”实体 | perception scope |
| O03 | 输入“听听外面” | 使用 hearing horizon、门和空间声学关系 | 因“外面”不是实体而拒绝；泄漏声源身份 | perception scope |
| O04 | 输入“感觉一下自己的身体” | 本体感觉/内感受投影 | 必须登记每个身体部位动词；补出未提交体征 | embodiment perception |
| O05 | “推门/用手把门顶开/肩膀抵着门向前挤” | 表面表达落到共享接触、施力和关系变化原语 | 三个专用句子 handler；忽略方法差异 | primitive compiler |
| O06 | “趴下来从门缝往外看” | 姿态、定向与 perception 按顺序结算 | 单一 look handler；未形成缝隙也看见外面 | composition integration |
| O07 | “把毯子卷起来塞到门缝下” | 候选只引用已落地对象与获准 effect schema；验证后产生局部关系/形态效果或诚实边界 | 模型直接写 Canon；仅因没有专用动词拒绝 | action proposal |
| O08 | 模型候选加入场景中不存在的胶带 | strict reject 该效果，零 Canon 或保留此前真实前缀 | 为完成玩家目标创建胶带 | proposal validator |
| O09 | 已知门锁死，玩家轻推 | 世界内物理失败/部分身体结果 | CAPABILITY_UNSUPPORTED；为有趣让锁失效 | adjudication |
| O10 | 玩家尝试穿墙到未建模街道 | 分开表达物理阻挡与世界边界，不伪造街道 | 把所有失败压成 TARGET_UNGROUNDED | boundary taxonomy |

正式集合现为原 50 项加 O01–O10，共 60。E01–E07 仍为不计数扩展池。

## 9. 纵向案例

V1–V3 是已经完成的可信内核纵切，不再单独构成可试玩 Demo 的准入证明。
...
覆盖 O01–O04：会话第一屏、无目标环顾、听觉范围和身体感知均从主体状态与世界关系投影。

### V5 未预编程组合行动

覆盖 O05–O10：使用通用原语和受验证 ActionProposal 完成至少一个开发时未逐句编码的组合行动，并保持所有可信内核门禁。

## 10. Goal 覆盖

| Demo Goal | 案例 |
|---|---|
| G1 自由 Attempt | I01–I10、O05–O10 |
...
1. 已完成 F/C/I/P/T/L 的可信内核基线；
2. O01–O04：先建立感知入口与无目标 scope；
3. O05–O08：建立通用原语、ActionProposal 和越权拒绝；
4. O09–O10：校验物理失败与各类边界；
5. V4/V5 通过后开展盲测自由会话；
6. 扩展池按真实 Demo 需要逐项启用。
...
- 60 个正式案例均能映射到 Runtime Protocol 阶段；
- G1–G13 全覆盖；
- V1–V3 已实现，V4/V5 有完整协议与实施计划；
- 不存在要求 LLM 拥有 Canon 写权限的案例；
- 编码计划按验收顺序拆分完成。

当前纠偏后的设计层退出条件尚未满足；下一步执行 Phase 8A，并用结果修订 O01–O10 的具体结构预期。
