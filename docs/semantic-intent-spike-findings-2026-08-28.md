...
**A2/A3 的核心方向值得继续投入 D2/D3 设计**，理由不是抽象论证，而是这次真实调用**逐字复现了两轮真人测试报告的两个具体 bug**，并且新 prompt 结构性地避免了它们；带干扰物的指代消解重测（见下）进一步证明持续世界语境不是"侥幸猜对"，而是真的能在"当前持有物"和"最近提及对象"冲突时选对后者。但也暴露了三个新 prompt 自身需要在设计阶段补的漏洞（下方"新 prompt 暴露的问题"）。这不是"架构已验证"，只是"值得把 D1–D4 写下去，而不是继续在纸面猜测"。
...
- **两轮指代消解案例结果最初不干净**，已用带干扰物的场景重测并解决——见下方"指代消解重测（带干扰物）"。
...
产物：`experiments/semantic-intent-spike/discourse-retest.mjs`，结果见 `results/discourse-retest.json`。

原案例的问题：六实体玩具世界里 `blanket-1` 是唯一带 `place`/`deformable` affordance 的物体，"它"猜对 `blanket-1` 可能只是"能被铺开的东西只有一个候选"，不能证明模型真的做了指代消解。重测加了一个**干扰物** `rug-1`（"那块卷起来的地毯"）：不持有、但同样带 `place`/`deformable` affordance，让"消去法"不再有免费的正确答案，并设计了两个子案例：

- **A（一致性）**：T1"看看手里有什么" → T2"把它铺开"，期望 `它 = blanket-1`（当前持有物）。
- **B（反消去法/新近性覆盖）**：T1"看看那块卷起来的地毯"（明确提到 rug-1，此时并未持有它）→ T2"把它铺开"，期望 `它 = rug-1`（最近被提及的物体），**而不是** `blanket-1`（当前持有物）。B 是关键测试：如果模型只是"无脑返回当前持有物"的启发式，B 会答错成 `blanket-1`；只有真正在追踪"最近说的是什么"，才会答对 `rug-1`。

结果：

- **A**：NEW 臂 T2 `roles.theme = "blanket-1"`，正确。
- **B**：NEW 臂 T1 正确识别 `theme = "rug-1"`；T2"它"解析为 `roles.theme = "rug-1"`——**正确地脱离了"当前持有物"默认值**，并且额外产出了一条我没有预设、但语义上合理的依赖：`{"role":"access","reason":"actor 当前持有 blanket-1，未持有 rug-1，需先获取才能铺开"}`，自己发现了"要铺开一个没拿着的东西，得先去拿"这个前置条件。
- OLD 臂：因为生产设计本来就是单轮无上下文调用，两个子案例的 T2 请求完全没有拿到 T1 的任何信息（这本身就如实反映了生产现状，不是测试疏漏）。结果两次都回答 `blanket-1`——A 蒙对，**B 蒙错**。这具体证明了"每轮失忆"在一个真实存在歧义的场景里会产生错误结果，不只是理论上的担忧。

**结论**：原先的警告已经解除。子案例 B 是干净的反例检验——模型不是靠"猜剩下的那个"或"总是选当前持有物"作弊，而是表现出了对"最近提及对象"的真实追踪，并能在其与"当前持有物"冲突时正确选择前者。唯一仍待关注的细节：Turn 1 用间接说法（"手里有什么"）时模型仍然没能把 `theme` 显式解析到具体 slot（这点在两次实验里稳定复现），T2 能答对更像是模型直接读取语境里 `holding` 关系做的兜底，而不是依赖我在脚本里传递的 `discourse.recentFocus`。这对整体结论没有影响（兜底路径本身就是持续世界语境该起的作用），但 D2 设计里仍应该要求"看看手里有什么"这类间接指称也显式产出可绑定的 `theme`，不能只靠运气对齐。
...
3. ~~在正式设计 D3 前，用一个带干扰物的场景重跑指代消解案例~~——已完成，见"指代消解重测（带干扰物）"，结论正面。D2 仍需补上"间接指称也必须显式产出可绑定 theme"这条要求。
4. 延迟问题需要单独立项处理（比如探索更小的 `max_completion_tokens`、更激进的 prompt 裁剪，或接受它作为已知产品约束），不属于这次架构方向要解决的范围。

## 原始数据

首轮完整 21 条请求/响应（含 latency、finishReason、usage、schema 校验结果）见 `experiments/semantic-intent-spike/results/latest.json`；指代消解重测的 6 条见 `results/discourse-retest.json`。
