...
`RealityDelta` 空时可以没有 Canon Commit；Wait 导致 World Time 或 Process 变化时通常不是空 Delta。
...
### 6.2 PROJECT_PREVIOUS

- 将上一 Height 已提交变化投影成当前主体可获得的 Observation；
...
首版没有通用骰子或随机结果。遇到规则不能唯一决定且不允许 Collapse 的情况，返回明确边界，不让 LLM自由挑选。
...
从 Finalized Delta 和最新 Reality 投影 Observation Seeds，不直接把 Delta 全量交给 Renderer。
...
默认不推进显著 World Time；若 Query 包含实际搜索/观察行为，应构成为 Attempt 或 Perception Operation，并计算时间。
