...
import {knownPropositions, cases} from "./cases.mjs";
import {REACHABILITY_SYSTEM_PROMPT, buildUserPrompt} from "./prompts.mjs";
  ...
  for (; attempts < 3; attempts += 1) {
    response = await fetch(
      `https://api.cloudflare.com/client/v4/accounts/${ACCOUNT_ID}/ai/run/${MODEL}`,
      {
        ...
        body: JSON.stringify({
          messages: [{role: "system", content: REACHABILITY_SYSTEM_PROMPT}, {role: "user", content: userPrompt}],
...
for (const [index, testCase] of cases.entries()) {
  ...
  try {
    const outcome = await callModel(buildUserPrompt(knownPropositions, testCase.claim));
    results.push({id: testCase.id, claim: testCase.claim, note: testCase.note, ...outcome});
    console.log("ok");
  } catch (error) {
    results.push({id: testCase.id, claim: testCase.claim, note: testCase.note, error: String(error)});
    console.log("ERROR");
  }
}

await mkdir(new URL("./results/", import.meta.url), {recursive: true});
const report = {generatedAt: new Date().toISOString(), accountId: ACCOUNT_ID, model: MODEL,
  knownPropositions, count: results.length, results};
await writeFile(new URL("./results/latest.json", import.meta.url), `${JSON.stringify(report, null, 2)}\n`);
console.log("Wrote experiments/reachability-inference-spike/results/latest.json");
